...
{
    public class CriancaExpressionBuilder<T> where T : class
    {
        ...
        public CriancaExpressionBuilder(string obj)
        {
        ...
        public CriancaExpressionBuilder<T> BuscarEmTudo(string? search)
        {
            if (!string.IsNullOrWhiteSpace(search))
            {
                search = search.Trim();

                var nomeProperty = Expression.Property(View, nameof(CriancaVm.NomeCompleto));
                var nome = Expression.Call(nomeProperty, MethodInfoContains!, Expression.Constant(search));

                var regiaoProperty = Expression.Property(View, nameof(CriancaVm.Regiao));
                ...
                var igrejaProperty = Expression.Property(View, nameof(CriancaVm.Igreja));
                ...
                {
                    var cargoLocalProperty = Expression.Property(View, nameof(CriancaVm.CargoLocal));
                ...
                {
                    var cargoRegionalProperty = Expression.Property(View, nameof(CriancaVm.CargoRegional));
