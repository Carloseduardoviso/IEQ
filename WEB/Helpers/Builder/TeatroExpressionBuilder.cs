...
{
    public class TeatroExpressionBuilder<T> where T : class
    {
        ...
        public TeatroExpressionBuilder(string obj)
        {
        ...
        public TeatroExpressionBuilder<T> BuscarEmTudo(string? search)
        {
            if (!string.IsNullOrWhiteSpace(search))
            {
                search = search.Trim();

                var nomeProperty = Expression.Property(View, nameof(TeatroVm.NomeCompleto));
                var nome = Expression.Call(nomeProperty, MethodInfoContains!, Expression.Constant(search));

                var regiaoProperty = Expression.Property(View, nameof(TeatroVm.Regiao));
                ...
                var igrejaProperty = Expression.Property(View, nameof(TeatroVm.Igreja));
                ...
                {
                    var cargoLocalProperty = Expression.Property(View, nameof(TeatroVm.CargoLocal));
                ...
                {
                    var cargoRegionalProperty = Expression.Property(View, nameof(TeatroVm.CargoRegional));
