...
{
    public class CasalExpressionBuilder<T> where T : class
    {
        ...
        public CasalExpressionBuilder(string obj)
        {
        ...
        public CasalExpressionBuilder<T> BuscarEmTudo(string? search)
        {
            if (!string.IsNullOrWhiteSpace(search))
            {
                search = search.Trim();

                var nomeProperty = Expression.Property(View, nameof(CasalVm.NomeCompleto));
                var nome = Expression.Call(nomeProperty, MethodInfoContains!, Expression.Constant(search));

                var regiaoProperty = Expression.Property(View, nameof(CasalVm.Regiao));
                ...
                var igrejaProperty = Expression.Property(View, nameof(CasalVm.Igreja));
                ...
                {
                    var cargoLocalProperty = Expression.Property(View, nameof(CasalVm.CargoLocal));
                ...
                {
                    var cargoRegionalProperty = Expression.Property(View, nameof(CasalVm.CargoRegional));
