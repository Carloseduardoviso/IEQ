    ...
    public class PastoresExpressionBuilder<T> where T : class
    {
        ...
        public PastoresExpressionBuilder<T> BuscarEmTudo(string? search)
        {
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                var nomeProperty = Expression.Property(View, nameof(PastoresVm.Nome));
                var nome = Expression.Call(nomeProperty, MethodInfoContains!, Expression.Constant(search));

                var regiaoProperty = Expression.Property(View, nameof(PastoresVm.Igreja));
                var regiaoNome = Expression.Property(regiaoProperty, nameof(IgrejaVm.Regiao));
                var regiaoNomeProperty = Expression.Property(regiaoNome, nameof(RegiaoVm.Nome));
                var regiaoContains = Expression.Call(regiaoNome, MethodInfoContains!, Expression.Constant(search));

                var igrejaProperty = Expression.Property(View, nameof(PastoresVm.Igreja));
